...
timeseriesCV_mediumBra <- function(workflow, fullDataset, initialConditions, numYears, thresholds, regionLUT){
  ...
  for (i in (1:numYears)){
    ...
    testEligGAULs <- train_data %>% dplyr::filter(Year_end==2001+i) %>% dplyr::filter(TestElig==1) %>% dplyr::select(GAUL)
    ...
    #Make predictions for all GAULS in test_data (5506 - # infected GAULS in initialConditions)
    ...
    newGAULPredictions <-  newGAULPredictionsDefaultThres %>% arrange(desc(.pred_1)) %>% 

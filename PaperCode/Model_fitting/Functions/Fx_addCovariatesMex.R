addCovariates_Mex <- function(GAULS_SOURCE, GAULS_DEST, spreadYear, fullDataset, admin1, admin2, location_data, regionLUT, big_cities = NULL){
  # spreadyear warning
  ...
  rtn_df <- data.frame(ae_suit = Mexico_Aegypti_sum[match(GAULS_DEST, admin2$GAUL_CODE), 1],
                       EVI_mean = suit.assemble(Mexico_EVI_sum, spreadYear, GAULS_DEST, admin2, FP = EVI_m_FP)[[1]],
                       EVI_stdDev = suit.assemble(Mexico_EVI_sum, spreadYear, GAULS_DEST, admin2, FP = EVI_sd_FP)[[2]],
                       Landcover = suit.assemble(Mexico_LandCover_sum, spreadYear, GAULS_DEST, admin2, FP = NA)[[1]],
                       LST_day_mean = suit.assemble(Mexico_LST_day_sum, spreadYear, GAULS_DEST, admin2, FP = LST_day_m_FP)[[1]],
                       LST_day_stdDev = suit.assemble(Mexico_LST_day_sum, spreadYear, GAULS_DEST, admin2, FP = LST_day_sd_FP)[[2]],
                       LST_night_mean = suit.assemble(Mexico_LST_night_sum, spreadYear, GAULS_DEST, admin2, FP = LST_night_m_FP)[[1]],
                       LST_night_stdDev = suit.assemble(Mexico_LST_night_sum, spreadYear, GAULS_DEST, admin2, FP = LST_night_sd_FP)[[2]],
                       TCB_mean = suit.assemble(Mexico_TCB_sum, spreadYear, GAULS_DEST, admin2, FP = NA)[[1]],
                       TCB_stdDev = suit.assemble(Mexico_TCB_sum, spreadYear, GAULS_DEST, admin2, FP = NA)[[2]],
                       TCW_mean = suit.assemble(Mexico_TCW_sum, spreadYear, GAULS_DEST, admin2, FP = NA)[[1]],
                       TCW_stdDev = suit.assemble(Mexico_TCW_sum, spreadYear, GAULS_DEST, admin2, FP = TCW_sd_FP)[[2]],
  ...
  #Add regions 
  admin1 <- admin1 #Just so we don't get an unused argument error 
  rtn_df$Region = ""
  ...
  #Add labels 
  spreadYearData <- fullDataset %>% dplyr::filter(Year_end == spreadYear)
  GAUL_index <- match(rtn_df$GAUL, spreadYearData$GAUL)
  rtn_df$Infected <- spreadYearData$Infected[GAUL_index]
